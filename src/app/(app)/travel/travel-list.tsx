...
export default function TravelList() {
  ...
  const articles = useMemo(() => {
    ...
    return allArticles.filter((article) => article.category === 'Location Spotlight');
  }, [allArticles]);
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      {articles.map((item) => (
       ...
       {articles?.length === 0 && (
         <p className="text-center text-muted-foreground py-8 col-span-full">No travel articles found.</p>
      )}
    </div>
  );
}
